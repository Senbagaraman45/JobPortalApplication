...
public class JobPostActivityController {
    ...
    public JobPostActivityController(UsersService usersService,JobPostActivityService
                                     activityService,JobSeekerApplyService seekerApplyService,
                                     JobSeekerSaveService seekerSaveService) {
        this.usersService = usersService;
        this.jobPostActivityService=activityService;
        this.jobSeekerApplyService=seekerApplyService;
        this.jobSeekerSaveService=seekerSaveService;
    }
    @GetMapping("/dashboard/")
    public String searchJobs(Model model, @RequestParam(value = "job",required = false)
                             ...
                             boolean days7,@RequestParam(value = "days30",required = false)
                             boolean days30){
        ...
        if(officeOnly==null&&remoteOnly==null&&freelance==null){
        ...
        if(!dateSearchFlag &&!remote &&!type &&!StringUtils.hasText(job) &&!StringUtils.hasText(location)){
            ...
            jobPost=jobPostActivityService.search(job,location, Arrays.asList(partTime,fullTime,
                    freelance),Arrays.asList(remoteOnly,officeOnly,partialRemote),searchDate);
        }
        ...
        if(!(authentication instanceof AnonymousAuthenticationToken)){
            ...
            if(authentication.getAuthorities().contains(new SimpleGrantedAuthority(
                    "Recruiter"))){
                List<RecruiterJobsDto>recruiterJobsDtos=jobPostActivityService.getRecruiterJobs(((RecruiterProfile)currentUserProfile)
                        .getUserAccountId());
    ...
    public String globalSearch(Model model, @RequestParam(value = "job",required = false)
                                    String job,@RequestParam(value = "location",required = false)
                                   String location,@RequestParam(value = "partTime",required = false)
                                   String partTime,@RequestParam(value = "fullTime",required = false)
                                   String fullTime,@RequestParam(value = "freelance",required = false)
